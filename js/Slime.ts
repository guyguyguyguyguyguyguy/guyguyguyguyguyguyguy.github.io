export interface Algorithm {
  getConfig(): { id: string; name: string; controls: any[] };
  setSourceImage(img: { data: Uint8ClampedArray; width: number; height: number }): void;
  reset(params?: Record<string, number>): void;
  updatesInPlace(): boolean;
  findBestRectangle(params: Record<string, number>): any;
  getRectangles(): any[];
  cullCoveredRectangles(): void;
  render(ctx: CanvasRenderingContext2D, params: Record<string, number>): void;
}
...
class SlimeMold {
  ...
  // Trail map for pheromone-like behavior
  trailMap: Float32Array;
  nextTrailMap: Float32Array;
  
  private W: number;
  private H: number;
  
  constructor(n: number, h: number, w: number) {
    this.count = n;
    this.W = w;
    this.H = h;
  ...
  update(brightness: Uint8ClampedArray, params: {
    sensorAngle: number,
    sensorDist: number,
    turnSpeed: number,
    moveSpeed: number,
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Move forward
      const newX = x + Math.cos(this.angle[i]) * moveSpeed * 0.01;
      const newY = y + Math.sin(this.angle[i]) * moveSpeed * 0.01;
...
export class Slime implements Algorithm {
  private img: { data: Uint8ClampedArray; width: number; height: number } | null = null;
  private sourceCanvas?: HTMLCanvasElement;
  private blurredCanvas?: HTMLCanvasElement;
  private blurredLast = { blur: -1 };
  private slimeMold: SlimeMold;
  private W = 0;
  private H = 0;
  private brightness: Uint8ClampedArray = new Uint8ClampedArray(0);

  // Trail visualization canvas
  ...
  setSourceImage(img: { data: Uint8ClampedArray; width: number; height: number }) {
    this.img = img;
    this.W = img.width;
    this.H = img.height;
    ...
    if (!this.blurredCanvas) this.blurredCanvas = document.createElement('canvas');
    this.blurredCanvas.width = this.W;
    this.blurredCanvas.height = this.H;
    this.blurredLast.blur = -1;
  
    if (!this.trailCanvas) this.trailCanvas = document.createElement('canvas');
  ...
  private updateBrightnessMap(blur: number) {
    if (!this.sourceCanvas || !this.blurredCanvas) return;
  ...
  reset(params?: Record<string, number>) {
  ...
  findBestRectangle(params: Record<string, number>) {
